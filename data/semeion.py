...
class Semeion(object):
    def __init__(self, P, tabular_size, seed, source, shot, tasks_per_batch, test_num_way, query):
        ...
        self.unlabeled_x = np.load('./data/semeion/train_x.npy')
        self.test_x = np.load('./data/semeion/xtest.npy')
        self.test_y = np.load('./data/semeion/ytest.npy')
        self.val_x = np.load('./data/semeion/val_x.npy')
        self.val_y = np.load(
            './data/semeion/yval.npy')  # val_y is given from pseudo-validaiton scheme with STUNT
