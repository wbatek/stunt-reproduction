...
class Karhunen(object):
    def __init__(self, P, tabular_size, seed, source, shot, tasks_per_batch, test_num_way, query):
        ...
        self.unlabeled_x = np.load('./data/karhunen/train_x.npy')
        self.test_x = np.load('./data/karhunen/xtest.npy')
        self.test_y = np.load('./data/karhunen/ytest.npy')
        self.val_x = np.load('./data/karhunen/val_x.npy')
        self.val_y = np.load(
            './data/karhunen/yval.npy')  # val_y is given from pseudo-validaiton scheme with STUNT
